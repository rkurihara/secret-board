...
const oneTimeTokenMap = new Map(); // key=ユーザ名,value=token
  ...
  switch (req.method) {
      ...
      case 'POST':　
        ...
        req.on('data', (chunk) => {
          ...
          const requestedOneTimeToken = dataArray[1] ? dataArray[1].split('onetTimeToke=')[1] : '';
...
function addTrackingCookie(cookies, userName) {
  ...
  if (isValidTrackingId(requestedTrackingId, userName)) {
    ...
    const trackingId = originalId + '_' + craeteValidHash(originalId, userName);
...
function isValidTrackingId(trackingId, userName) {
  ...
  return craeteValidHash(originalId, userName) === requestedHash;
}
...
function craeteValidHash(originalId, userName) {
